# Agentic Tool 설계: 동기면담(MI) 표준화 환자 시뮬레이션

## 1. 에이전트 핵심 페르소나 (Role)
*   **역할**: 동기면담(MI) 훈련을 돕는 표준화 환자 (Standardized Patient).
*   **목표**: 상담가(사용자)의 질문에 반응하며, 변화에 대해 양가감정을 가진 환자로서 실시간 음성 대화를 수행함.
*   **동작 원리**:
...
*   **Prompt**: "환자의 이름, 상담 주제(예: 금연, 당뇨 관리), 변화에 대한 저항 수준(1~10)을 설정해줘. 설정이 완료되면 [시작]이라고 말해줘."

### Phase 2: 시뮬레이션 (Simulation - Gemini Live)
환자 역할을 수행하며 상담가와 대화합니다.
*   **System Instruction**:
...
*   **Analysis Prompt**:
    ...
    > 3. **결정적 장면**: 환자가 변화의 의지를 보인 순간(Change Talk)에 상담가가 어떻게 반응했는지 리뷰하라.
    > 4. **감정 피드백**: 상담가의 목소리 톤이 환자에게 어떻게 느껴졌는지(권위적/공감적 등) 평가하라.
