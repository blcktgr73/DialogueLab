...
- **Change (변경 사항)**:
  - STT 요청 설정을 공통 유틸(`src/lib/stt-config.ts`)로 분리.
  - 모델/화자 수/언어/샘플레이트를 env로 제어 가능하도록 구성.
  - STT 설계 문서에 env 항목 추가.
- **Constraints (제약 사항)**:
  - env 값이 잘못될 경우 요청 실패 위험 → 유효성 검사 및 경고 로그 추가.
- **Design Options (설계 옵션)**:
  - (A) 코드 상수 고정: 단순하지만 실험 비용 높음.
  - (B) env 기반 설정 분리: 반복 튜닝 용이 (선택).
- **Chosen & Rationale (선택 및 근거)**:
  - (B) 실험 속도와 운영 유연성을 우선.
- **Acceptance (테스트/데모 기준)**:
  - env 값 변경 후 STT 요청이 해당 설정을 반영.
- **Impact (API/Data/UX/문서 영향)**:
  - 문서: STT env 목록 업데이트.
- **Structural Quality Metric Change (구조적 품질 지표 변화)**:
  - 응집도: STT 설정 책임이 유틸로 분리되어 응집도 증가.
  - 결합도: 코드-설정 결합도 감소.
- **Follow-ups (후속 작업)**:
  - 한국어 샘플로 모델/화자수 조합 실험 및 권장값 확정.
